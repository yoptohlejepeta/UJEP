...
p <- ggplot(data = women, aes(x = height, y = weight)) + theme_light() +
...
p1 <- ggplot(data = mydata, aes(sample = resid_1))+ theme_light() +
  stat_qq() + stat_qq_line() +
  labs(title = 'a) Normal Q-Q plot, simple linear regression', x = 'Theoretical quartiles',
       y = 'Residuals') +
  theme(plot.title = element_text(size = rel(1.3), hjust = 0.5), 
        axis.title = element_text(size = rel(1.1)),axis.text = element_text(size = rel(1.1)),
        legend.position = 'NA')
p2 <- ggplot(data = mydata, aes(sample = resid_2))+ theme_light() +
  stat_qq() + stat_qq_line() +
  labs(title = 'b) Normal Q-Q plot, polynomial linear regression', x = 'Theoretical quartiles', 
       y = 'Residuals') + 
  theme(plot.title = element_text(size = rel(1.3), hjust = 0.5), 
        axis.title = element_text(size = rel(1.1)),axis.text = element_text(size = rel(1.1)),
        legend.position = 'NA')
p3 <- ggplot(data = mydata, aes(sample = resid_3))+ theme_light() +
  stat_qq() + stat_qq_line() +
  labs(title = 'c) Normal Q-Q plot, polynomial linear regression', x = 'Theoretical quartiles', 
       y = 'Residuals') + 
  theme(plot.title = element_text(size = rel(1.3), hjust = 0.5), 
        axis.title = element_text(size = rel(1.1)),axis.text = element_text(size = rel(1.1)),
        legend.position = 'NA')

p4 <- ggplot(data = mydata, aes(x = height)) + theme_light() +
  geom_point(aes(y = weight, fill = 'black')) + geom_line(aes(y = pred1, colour = 'red')) + 
  labs(title = 'd) Simple linear regression', x = 'Height (inch)', y = 'Weight (lbs)') +
  theme(plot.title = element_text(size = rel(1.3), hjust = 0.5), 
        axis.title = element_text(size = rel(1.1)),axis.text = element_text(size = rel(1.1)),
        legend.position = 'NA')
p5 <- ggplot(data = mydata, aes(x = height)) + theme_light() +
  geom_point(aes(y = weight, fill = 'black')) + geom_line(aes(y = pred2, colour = 'red')) + 
  labs(title = 'e) Polynomial linear regression', x = 'Height (inch)', y = 'Weight (lbs)') +
  theme(plot.title = element_text(size = rel(1.3), hjust = 0.5), 
        axis.title = element_text(size = rel(1.1)),axis.text = element_text(size = rel(1.1)),
        legend.position = 'NA')
p6 <- ggplot(data = mydata, aes(x = height)) + theme_light() +
  geom_point(aes(y = weight, fill = 'black')) + geom_line(aes(y = pred3, colour = 'red')) + 
...
p <- ggplot(data = mydata, aes(x = conc, y = rate)) + theme_light() +
  geom_point() + 
  labs(title = 'Scatter plot rate vs conc', x = 'Conc', y = 'Rate') +
  theme(plot.title = element_text(size = rel(1.3), hjust = 0.5), 
        axis.title = element_text(size = rel(1)),axis.text = element_text(size = rel(1)))

png(file = 'nonlinear_data.png', width = 2000, height = 1600,
    res = 600)
print(p)
dev.off()
...
p1 <- ggplot(data = mydata, aes(x = conc)) + theme_light() +
  geom_point(aes(y = rate)) + geom_line(aes(y = r_1, colour = 'linear')) +
  geom_line(aes(y = r_2, colour = 'polynomial')) + geom_line(aes(y = r_3, colour = 'nonlinear')) +
  scale_color_manual(name = "Models", values = c("linear" = "red", "polynomial" = "green", 
                                                 "nonlinear" = "blue"))+
  labs(title = 'a)Regression models: rate vs conc', x = 'Conc', y = 'Rate') +
  theme(plot.title = element_text(size = rel(1.3), hjust = 0.5), 
        axis.title = element_text(size = rel(1)),axis.text = element_text(size = rel(1)))

p2 <- ggplot(data = mydata, aes(sample = resid_3))+ theme_light() +
  stat_qq() + stat_qq_line() +
...
p3 <- ggplot(data = mydata, aes(sample = resid_4))+ theme_light() +
  stat_qq() + stat_qq_line() +
...
p4 <- ggplot(data = mydata, aes(sample = resid_5))+ theme_light() +
  stat_qq() + stat_qq_line() +
...
png(file = 'Nonlinear_regression_Results.png', width = 6000, height = 4000,
    res = 600)
